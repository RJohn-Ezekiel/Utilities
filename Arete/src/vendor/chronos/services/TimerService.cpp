...
void TimerService::startCustom(const QString& label, int minutes)
{
    m_currentTaskId.clear();
    m_customLabel = label;
    m_engine->start(SessionType::Focus, qBound(1, minutes, 480) * 60);
    saveSessionState();
}
...
QString TimerService::recordSession(SessionType type, int durationSeconds, bool completed)
{
    ...
    if (type == SessionType::Focus && !m_customLabel.isEmpty()) {
        session.note = m_customLabel;
        m_customLabel.clear();
    }
